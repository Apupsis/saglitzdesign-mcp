...
**One purpose, one opt-in.** The consent guidelines: "a controller that seeks consent for various different purposes should provide a separate opt-in for each purpose, to allow users to give specific consent for specific purposes." Bundling analytics with advertising in one switch fails granularity even if the switch defaults off.
...
> **Myth check — "the buttons must be visually identical" is not what the regulator said (verified 2026-08-12).** The Cookie Banner Taskforce agreed "that a general banner standard concerning colour and/or contrast cannot be imposed on data controllers," and required case-by-case assessment of whether colours and contrast are "obviously misleading". The one example it called manifestly misleading is extreme: a reject button "where the contrast between the text and the button background is so minimal that the text is **unreadable** to virtually any user." So the EDPB's *legal* floor is lower than the design rule. Ship the design rule anyway — equal weight, equal size, equal prominence — because it is the position CPPA-style symmetry rules and [[ethical-design]] both take (California's § 7004(a)(2) is not confined to path length: its subparagraphs name a more prominent accept button as not equal or symmetrical, and [[ethical-design]] carries the quotation), because a case-by-case assessment is a thing you lose slowly and expensively, and because a ghost-button reject fails contrast requirements in [[accessibility]] on its own merits. What no regulator requires is buttons that are *pixel-identical*.
...
- **You cannot swap the legal basis when consent becomes inconvenient.** The EDPB: "the controller cannot swap from consent to other lawful bases," and a change of basis has to be notified to the user. Deciding at collection time, per field, is what makes the deletion screen implementable at all.
