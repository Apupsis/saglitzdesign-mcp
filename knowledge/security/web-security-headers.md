...
- [ ] `object-src 'none'`, `base-uri 'none'`, `frame-ancestors`, `form-action 'self'` all present — `base-uri`, `frame-ancestors` and `form-action` have **no** `default-src` fallback ("Not setting this allows anything"), and `object-src` does fall back to `default-src` but is still worth stating, because that leaves plugin execution riding on whatever `default-src` happens to be rather than on a decision
