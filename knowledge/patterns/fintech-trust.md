...
- **Progressive disclosure of the burden.** Let people explore value before hitting the hardest steps. Collect identity data in the smallest sequential chunks that make sense, with a visible step indicator ([[principles-heuristics]] via [[onboarding-permission-priming]]). Never dump a 20-field compliance form on screen one.
