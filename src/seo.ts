...
export const SEO_NOT_VISIBLE: string[] = [
  ...
  "**…and the exemption is narrower than \"exempted from the page rules\".** What a recognised email skips is the head-absence set — `title-missing`, `meta-description-missing`, `canonical-missing` — because those are claimed only for a self-contained document. `multiple-h1`, `alt-missing`, `heading-order-skipped` and the JSON-LD rules still grade it, and `title-length` still grades a subject line, but only its over-60-character half: a short one is not flagged, because that half of the rule is itself gated on a self-contained document, which a recognised email never is — so a two-character `<title>` on an email passes silently while the same string on a page draws the warning, and only a subject line long enough to be truncated in a client's preview still fires.",
